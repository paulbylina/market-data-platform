...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit(
            "Usage: uv run python src/features/tick/build_L1_1s_trade_quote_summary.py "
    ...
    summary = (
        df.group_by_dynamic("ts_utc", every="1s", closed="left", label="left")
        .agg(
            [
        ...
        .rename({"ts_utc": "second_start_utc"})
        .with_columns(
            [
                pl.col("second_start_utc")
                .dt.convert_time_zone("America/Chicago")
                .alias("second_start_central"),
            ]
        )
        .with_columns(
            [
                pl.col("second_start_central").dt.date().alias("trade_date_central"),
                pl.col("second_start_central").dt.hour().alias("hour_central"),
                pl.col("second_start_central").dt.minute().alias("minute_central"),
                pl.col("second_start_central").dt.second().alias("second_central"),
                (pl.col("last_ask_price") - pl.col("last_bid_price")).alias("ending_spread"),
    ...
    print(f"Wrote 1-second trade/quote summary parquet: {output_path}")
