...
FIRST_BAR_FILTERS = [
    ("no_first_bar_filter", None, None),
    ("first_bar_ge_0", ">=", 0.0),
    ("first_bar_ge_0_25", ">=", 0.25),
    ("first_bar_ge_0_50", ">=", 0.50),
    ("first_bar_ge_0_75", ">=", 0.75),
    ("first_bar_le_0", "<=", 0.0),
...
def simulate_short_stop_target_eod(rth: pd.DataFrame) -> dict:
    ...
    for _, bar in rth.iloc[1:].iterrows():
        # Conservative assumption for same-bar ambiguity:
        # for a short, if stop and target are both touched in the same bar,
...
def passes_first_bar_filter(series: pd.Series, operator: str | None, threshold: float | None) -> pd.Series:
    if operator is None:
...
def main() -> None:
    ...
    for _, row in candidates.iterrows():
        ...
        result = simulate_short_stop_target_eod(rth)
    ...
    for rel_gap in REL_GAP_THRESHOLDS:
        for rank_limit in RANK_LIMITS:
            for filter_name, operator, threshold in FIRST_BAR_FILTERS:
                ...
                mask = mask & passes_first_bar_filter(
    ...
    results_path = OUTPUT_DIR / "no_oversold_relative_gap_short_results.csv"
    summary_path = OUTPUT_DIR / "no_oversold_relative_gap_short_summary.csv"

    results.to_csv(results_path, index=False)
    summary.to_csv(summary_path, index=False)

    print("=== No-oversold relative-gap SHORT test ===")
