...
for symbol in symbols:
    ...
    df = (
        ...
        .merge(spy[["date", "spy_close", "spy_return", "spy_zscore_200d"]], on="date", how="left")
        .sort_values("date")
...
def build_panel_for_split(train_end: str) -> pd.DataFrame:
    ...
    for symbol in symbols:
        ...
        df["volume_signal"] = df["volume_ratio_20d"] >= RVOL20_MIN
...
def simulate_strategy(panel: pd.DataFrame, dates):
    ...
    for date in dates:
        ...
        day_signals = day_all[
            ...
            & (day_all["volume_signal"])
        ].copy()

        # Candidate priority: most oversold first.
...
for split in SPLITS:
    ...
    strategy_df, trades_df = simulate_strategy(split_panel, oos_dates)
    ...
    summary_rows.append(
        {
            ...
            "strategy_dd_minus_spy_dd": strategy_dd - spy_dd,
...
avg = pd.DataFrame(
    [
        {
            "avg_strategy_return": summary["strategy_return"].mean(),
            "avg_spy_buy_hold_return": summary["spy_buy_hold_return"].mean(),
            "avg_equal_weight_basket_return": summary["equal_weight_basket_return"].mean(),
            "avg_strategy_minus_spy": summary["strategy_minus_spy"].mean(),
            "min_strategy_minus_spy": summary["strategy_minus_spy"].min(),
            "avg_strategy_minus_basket": summary["strategy_minus_basket"].mean(),
            "min_strategy_minus_basket": summary["strategy_minus_basket"].min(),
            "avg_strategy_max_dd": summary["strategy_max_dd"].mean(),
            "avg_spy_max_dd": summary["spy_max_dd"].mean(),
            "avg_basket_max_dd": summary["basket_max_dd"].mean(),
            ...
            "avg_active_weight": summary["avg_active_weight"].mean(),
            "max_active_weight": summary["max_active_weight"].max(),
        }
    ]
)

print("=== Final candidate vs SPY buy-and-hold ===")
print("Candidate: baseline oversold pullback + SPY regime + RVOL20 >= 1.2")
