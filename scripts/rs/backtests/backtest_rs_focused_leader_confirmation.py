...
TESTS = [
    ...
    {
        "name": "sector_200d_top_30",
        "sector_200d_cutoff": 0.30,
        "sector_1d_cutoff": None,
        "priority": "oversold",
    },
    {
        "name": "sector_1d_top_20",
        "sector_200d_cutoff": None,
        "sector_1d_cutoff": 0.20,
        "priority": "oversold",
    },
    {
        "name": "sector_1d_top_30",
        "sector_200d_cutoff": None,
        "sector_1d_cutoff": 0.30,
        "priority": "oversold",
    },
    {
        "name": "sector_200d_top_30_and_1d_top_20",
        "sector_200d_cutoff": 0.30,
        "sector_1d_cutoff": 0.20,
        "priority": "oversold",
    },
    {
        "name": "sector_200d_top_30_and_1d_top_30",
        "sector_200d_cutoff": 0.30,
        "sector_1d_cutoff": 0.30,
        "priority": "oversold",
    },
    {
        "name": "baseline_1d_priority",
        "sector_200d_cutoff": None,
        "sector_1d_cutoff": None,
        "priority": "sector_1d",
    },
    {
        "name": "sector_200d_top_30_1d_priority",
        "sector_200d_cutoff": 0.30,
        "sector_1d_cutoff": None,
        "priority": "sector_1d",
    },
...
def apply_test_filters(day_signals: pd.DataFrame, test: dict) -> pd.DataFrame:
    out = day_signals.copy()

    if test["sector_200d_cutoff"] is not None:
        out = out[
            out["sector_stock_return_200d_rank_pct"] <= test["sector_200d_cutoff"]
        ].copy()

    if test["sector_1d_cutoff"] is not None:
        out = out[
            out["sector_stock_return_1d_rank_pct"] <= test["sector_1d_cutoff"]
        ].copy()

    return out


def sort_signals(day_signals: pd.DataFrame, test: dict) -> pd.DataFrame:
    if test["priority"] == "sector_1d":
...
def simulate_basket_replacement(
    ...
    for date in dates:
        ...
        day_signals = apply_test_filters(day_signals, test)
        ...
        day_signals = sort_signals(day_signals, test)
...
print("=== Focused leader + confirmation test ===")
...
print(f"Max positions tested: {MAX_POSITIONS_LIST}")
print()
...
print("\n=== Average by test / hold / max positions across WF1 + WF2 ===")
avg = (
    summary
    .groupby(["test", "hold_days", "max_positions", "max_signal_exposure"])
    .agg(
...
pivot = (
    summary
    .pivot_table(
        index=["test", "hold_days", "max_positions", "max_signal_exposure"],
        columns="split",
...
print(
    summary
    .round(4)
    .sort_values(["split", "hold_days", "max_positions", "excess_return"], ascending=[True, True, True, False])
    .to_string(index=False)
)
