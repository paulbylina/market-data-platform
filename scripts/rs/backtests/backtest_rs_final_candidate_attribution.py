...
def simulate_candidate(panel: pd.DataFrame, dates, candidate: dict):
                ...
                trades.append(
                    {
                        ...
                        "exit_date": date,
                        "hold_days": hold_days,
                        "entry_close_zscore_50d": pos["entry_close_zscore_50d"],
                        "entry_sector_1d_rank_pct": pos["entry_sector_1d_rank_pct"],
                        "entry_sector_200d_rank_pct": pos["entry_sector_200d_rank_pct"],
            ...
            open_positions.append(
                {
                    ...
                    "entry_close_zscore_50d": row["close_zscore_50d"],
                    "entry_sector_1d_rank_pct": row["sector_stock_return_1d_rank_pct"],
                    "entry_sector_200d_rank_pct": row["sector_stock_return_200d_rank_pct"],
                }
            )
...
def summarize(split_name, candidate, equity_df, trades_df, basket_return, basket_dd):
    ...
    return {
        ...
        "hold_days": candidate["hold_days"],
        "max_positions": candidate["max_positions"],
        "max_signal_exposure": candidate["max_positions"] * POSITION_SIZE * 100,
        "trades": trades,
...
avg = (
    summary
    .groupby(["candidate", "hold_days", "max_positions", "max_signal_exposure"])
    .agg(
...
print(
    avg
    .round(4)
    .sort_values(["avg_excess_return", "min_excess_return"], ascending=[False, False])
    .to_string(index=False)
)

if len(trades) > 0:
    print("\n=== Sector attribution by candidate ===")
    sector_attr = (
        trades
        .groupby(["candidate", "sector"])
        .agg(
            trades=("ticker", "count"),
            avg_trade=("stock_trade_return_pct", "mean"),
            median_trade=("stock_trade_return_pct", "median"),
            win_rate=("stock_trade_return_pct", lambda s: (s > 0).mean() * 100),
            basket_excess_contribution=("basket_excess_contribution_pct", "sum"),
        )
        .reset_index()
    )
    ...
    print(
        ...
        .round(4)
        .sort_values(
            ["candidate", "basket_excess_contribution"],
            ascending=[True, False],
        )
        .to_string(index=False)
    )

    print("\n=== Top ticker attribution by candidate ===")
    ticker_attr = (
        trades
        .groupby(["candidate", "ticker", "sector"])
        .agg(
            trades=("ticker", "count"),
            avg_trade=("stock_trade_return_pct", "mean"),
            median_trade=("stock_trade_return_pct", "median"),
    ...
    for candidate_name in [c["name"] for c in CANDIDATES]:
        sub = ticker_attr[ticker_attr["candidate"] == candidate_name].copy()

        print(f"\n--- {candidate_name}: top 15 tickers ---")
        print(
            sub
            .round(4)
            .sort_values("basket_excess_contribution", ascending=False)
            .head(15)
            .to_string(index=False)
        )

        print(f"\n--- {candidate_name}: bottom 15 tickers ---")
        print(
            sub
            .round(4)
            .sort_values("basket_excess_contribution", ascending=True)
            .head(15)
            .to_string(index=False)
        )
