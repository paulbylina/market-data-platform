...
TASKS_PATH = Path(
    "data/research/full_market_scanner_10y/daily_regime_event_tasks/high_price_short_fade_pilot_tasks.csv"
)
...
def fetch_1m_bars(ticker, start_date, end_date, api_key, sleep_seconds=0.15):
    ...
    for attempt in range(1, 6):
        ...
        if r.status_code == 200:
            ...
            # Massive/Polygon aggregate fields:
            # t timestamp ms, o/h/l/c, v volume, vw vwap, n transactions
            rename = {
                "t": "timestamp_ms",
                "o": "open",
                "h": "high",
        ...
        if r.status_code in {429, 500, 502, 503, 504}:
            ...
            print(f"retry {attempt} {ticker} {start_date}->{end_date}: status {r.status_code}, wait {wait}s")
            time.sleep(wait)
            continue
...
def main():
    ...
    features_path = OUTPUT_DIR / "high_price_short_fade_features.csv"
    ...
    summary.to_csv(OUTPUT_DIR / "high_price_short_fade_features_summary.csv", index=False)
    print()
    print("saved:", OUTPUT_DIR / "high_price_short_fade_features_summary.csv")
